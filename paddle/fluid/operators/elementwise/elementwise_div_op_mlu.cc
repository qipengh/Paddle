...
class ElementwiseDivGradMLUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    //   // call Reciprocal op to compute 1/y
    //   Tensor tmp_y(y->type());
    //   tmp_y.mutable_data<T>(y->dims(), ctx.GetPlace());
    //   MLUUnaryOp<RECIPROCAL, T>(ctx, *y, &tmp_y);
    //   MLUCnnlTensorDesc tmp_y_desc(max_dim, y_dims_array.data(),
    //   ToCnnlDataType<T>());
    ...
    //     MLUCnnl::OpTensor(ctx, mul_op_desc.get(), dout_desc.get(),
    //                       GetBasePtr(dout), tmp_y_desc.get(),
    //                       GetBasePtr(&tmp_y),
    ...
    // if (dy) {
    //   // compute dy = -out/y * dout
    //   dy->mutable_data<T>(ctx.GetPlace());

    //   // call Neg and div op to compute -out/y
    //   Tensor neg_out(out->type());
    //   neg_out.mutable_data<T>(out->dims(), ctx.GetPlace());
    //   MLUUnaryOp<NEG, T>(ctx, *out, &neg_out);

    //   Tensor dy_temp(y->dtype());
    //   dy_temp.Resize(dout->dims());
    //   dy_temp.mutable_data<T>(ctx.GetPlace());
    //   MLUBinary<DIV>(ctx, CNNL_COMPUTATION_HIGH_PRECISION, dout_desc.get(),
    //       GetBasePtr(&neg_out), y_desc.get(), GetBasePtr(y), dout_desc.get(),
    //       GetBasePtr(&dy_temp));

    //   if (dy->dims() != dout->dims()) {
    //     MLUCnnl::OpTensor(ctx, mul_op_desc.get(), dout_desc.get(),
    //                       GetBasePtr(dout), dout_desc.get(),
    //                       GetBasePtr(&dy_temp),
