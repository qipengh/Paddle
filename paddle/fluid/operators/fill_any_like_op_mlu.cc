...
class FillAnyLikeMLUKernel : public framework::OpKernel<T> {
 public:
  ...
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    MLUCnnl::Fill(ctx, value, out_desc.get(), GetBasePtr(out));
  }
};
...
REGISTER_OP_NPU_KERNEL(fill_any_like, ops::FillAnyLikeMLUKernel<int>,
#ifdef PADDLE_WITH_ASCEND_INT64
                       ops::FillAnyLikeMLUKernel<int64_t>,
#endif
                       ops::FillAnyLikeMLUKernel<float>,
                       ops::FillAnyLikeMLUKernel<paddle::platform::float16>);
